...
// Função auxiliar para remover acentos de qualquer texto
...
const RegistroLesoes = ({ onSessionExpired }) => {
  ...
  const abrirEditar = (lesao) => {
    ...
    setFormLesao({
      ...
      tempoRecuperacao:
        lesao.tempoRecuperacao != null ? String(lesao.tempoRecuperacao) : "",
  ...
  const handleSave = async (e) => {
    ...
    const payload = {
      ...
      tempoRecuperacao:
        formLesao.tempoRecuperacao === ""
          ? null
          : Number(formLesao.tempoRecuperacao),
  ...
  // Funções de formatação movidas para ANTES do useMemo
  ...
  const lesoesFiltradas = useMemo(() => {
    // Normaliza o texto buscado (remove acentos e deixa em caixa baixa)
    const termoBuscaNormalizado = removerAcentos(busca).trim().toLowerCase();
    ...
    return lesoes.filter((lesao) => {
      const gravidadeLabel = getLabelGravidade(lesao.gravidade);
      const statusLabel = getLabelStatus(lesao.statusRecuperacao);

      return [lesao.jogadorNome, lesao.tipoLesao, gravidadeLabel, statusLabel]
        .filter(Boolean)
        .some((value) => {
          // Normaliza os valores da tabela antes de comparar
          const valorNormalizado = removerAcentos(value).toLowerCase();
          return valorNormalizado.includes(termoBuscaNormalizado);
        });
    });
  }, [busca, lesoes]);
  ...
  const getCorGravidade = (gravidade) => {
    switch (gravidade) {
      case "GRAVE":
        return { bg: "#ef444420", text: "#fca5a5", border: "#ef4444" };
      case "MODERADA":
        return { bg: "#f59e0b20", text: "#fcd34d", border: "#f59e0b" };
      default:
        return { bg: "#10b98120", text: "#6ee7b7", border: "#10b981" };
    }
  };
  ...
  return (
    <div style={styles.container}>
      <div style={styles.cardsGrid}>
        ...
        <div style={styles.card}>
          ...
          <h3 style={styles.cardValue}>
            {resumo?.retornoPrevistoProximos7Dias ?? 0}
          </h3>
          <span style={{ color: "#94a3b8", fontSize: "12px" }}>
      ...
      {loading ? (
        <div style={styles.loadingBox}>Carregando lesoes...</div>
      ) : null}
            ...
            {lesoesFiltradas.map((lesao) => {
              ...
              return (
                <tr key={lesao.id} style={styles.tableRow}>
                  ...
                  <td style={styles.td}>
                    <span
                      style={{
                        ...
                        color: cores.text,
                        border: `1px solid ${cores.border}50`,
                      }}>
                  ...
                  <td style={styles.td}>
                    {formatarData(lesao.dataPrevistaRetorno)}
                  </td>
                  <td style={styles.td}>
                    {getLabelStatus(lesao.statusRecuperacao)}
                  </td>
                  <td style={styles.tdActions}>
                    <button
                      style={styles.actionBtn}
                      title='Editar'
                      onClick={() => abrirEditar(lesao)}>
                      <Edit size={18} color='#10b981' />
                    </button>
                    <button
                      style={styles.actionBtn}
                      title='Remover Registro'
                      onClick={() => handleDelete(lesao.id)}>
          ...
          <form onSubmit={handleSave} style={styles.modalContent}>
            <div style={styles.modalHeader}>
              <h3 style={styles.modalTitle}>
                {modoModal === "novo"
                  ? "Registrar Nova Lesao"
                  : `Editar Lesao #${formLesao.id}`}
              </h3>
              <button
                type='button'
              ...
              <div
                ...
                <div>
                  <label style={styles.fieldLabel}>
                    Diagnostico / Tipo de Lesao
                  </label>
                  <input
                    type='text'
                    name='tipoLesao'
                    required
                    value={formLesao.tipoLesao}
                    onChange={handleChange}
                    placeholder='Ex: Estiramento grau 2 na coxa'
                    style={styles.inputModal}
                  />
                </div>
                <div style={{ display: "flex", gap: "15px" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <select
                      name='statusRecuperacao'
                      value={formLesao.statusRecuperacao}
                      onChange={handleChange}
                      style={styles.inputModal}>
                ...
                <div style={{ display: "flex", gap: "15px" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <label style={styles.fieldLabel}>
                      Tempo de Recuperacao (dias)
                    </label>
                    <input
                      type='number'
                      name='tempoRecuperacao'
                      min='0'
                      value={formLesao.tempoRecuperacao}
                      onChange={handleChange}
                      style={styles.inputModal}
                    />
                  </div>
